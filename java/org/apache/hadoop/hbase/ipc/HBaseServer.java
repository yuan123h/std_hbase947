...
public abstract class HBaseServer implements RpcServer {
  ...
  protected class Call implements RpcCallContext {
    ...
    /**
     * call 是由  {@link Connection} 实例化的，同时  Call  持有 它的引用
     * @param id
     * @param param
     * @param connection
     * @param responder
  ...
  private class Listener extends Thread {
    ...
    private Reader[] readers = null; //持有reader 线程的引用
    ...
    /**
     * 使用 java nio 技术 读取数据，封装 {@link Connection} 对象
     * @author yuanhuan
     *
     */
    private class Reader implements Runnable {
      ...
      /**
       * 不断的轮询，有等待机制：  等待一分钟
       * 轮询的控制 由 其母体类 {@link HBaseServer} 中的参数 控制
       */
  ...
  /** Reads calls from a connection and queues them for handling. </br>
   * 构造 {@link Call} 。 谁构造的 它自己呢？ {@link org.apache.hadoop.hbase.ipc.HBaseServer.Listener.Reader}
   * 这个类 读取出来的
   * 
   */
   ...
   * Client will get {@link ServerNotRunningYetException} instead. </br>
   * 开服务进程 </br>
   * 启动 {@link Listener} 实例化对象的进程
   */
  @Override
